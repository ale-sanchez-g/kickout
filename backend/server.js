...
function normalizeVertexRequestBody(rawBody, apiClientName) {
  if (!rawBody || !apiClientName.includes('VertexGenAi:')) {
    return rawBody;
  }
  ...
  if (typeof rawBody === 'string') {
    ...
    try {
      ...
      // If it is not JSON, forward unchanged.
      return rawBody;
    }
  ...
  if (parsedBody?.systemInstruction && typeof parsedBody.systemInstruction === 'object') {
    const role = parsedBody.systemInstruction.role;
    // Vertex expects systemInstruction as system content; user role here can trigger request validation errors.
    if (role && role !== 'system') {
      delete parsedBody.systemInstruction.role;
    }
  }

  return inputWasString ? JSON.stringify(parsedBody) : parsedBody;
}

function pickForwardableHeaders(incomingHeaders = {}) {
  ...
  return Object.fromEntries(
    Object.entries(incomingHeaders).filter(([key]) => !blockedHeaders.has(String(key).toLowerCase()))
  );
}
...
app.post('/api-proxy', async (req, res) => {
  ...
  try {
    ...
    const forwardedHeaders = pickForwardableHeaders(headers);
    const normalizedBody = normalizeVertexRequestBody(body, apiClient.name);

    const apiFetchOptions = {
      method: method || 'POST',
      headers: {...forwardedHeaders, ...apiHeaders},
      body: normalizedBody ? normalizedBody : undefined,
    };
    ...
    if (apiClient.isStreaming) {
      ...
      if (!apiResponse.ok) {
        console.error(`[Node Proxy] Upstream returned ${apiResponse.status} for ${apiClient.name}`);
        if (rawBody) {
      ...
      if (parsedBody !== null) {
        res.status(apiResponse.status).json(parsedBody);
      } else {
        res
          .status(apiResponse.status)
          .type('text/plain')
          .send(rawBody || `Upstream returned status ${apiResponse.status} with empty body.`);
      }
    }
  } catch (error) {
