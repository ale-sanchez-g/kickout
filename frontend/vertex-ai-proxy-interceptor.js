...
(function() {
  ...
  const configuredBackendBase = (import.meta?.env?.VITE_BACKEND_URL || '').trim().replace(/\/$/, '');

  function getProxyHttpUrl() {
    return configuredBackendBase ? `${configuredBackendBase}/api-proxy` : '/api-proxy';
  }

  function getProxyWsBaseUrl() {
    if (!configuredBackendBase) {
      const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
      return `${protocol}//${window.location.host}/ws-proxy`;
    }

    try {
      const backendUrl = new URL(configuredBackendBase);
      const wsProtocol = backendUrl.protocol === 'https:' ? 'wss:' : 'ws:';
      return `${wsProtocol}//${backendUrl.host}/ws-proxy`;
    } catch {
      const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
      return `${protocol}//${window.location.host}/ws-proxy`;
    }
  ...
  window.WebSocket = function(url, protocols) {
    ...
    if (inputUrl && isValidUrl(inputUrl)) {
      ...
      const proxyUrl = `${getProxyWsBaseUrl()}?target=${targetUrl}`;
      ...
      try {
        ...
        const proxyFetchOptions = {
          ...
          headers: {
            ...
            'X-App-Proxy': '6bp5n7mtRMSZkjcJ7mYBkksvHeOqBXzP',
          },
          body: JSON.stringify(requestDetails),
        };

        const proxyHttpUrl = getProxyHttpUrl();
        console.log(`[Vertex AI Proxy Shim] Fetching from Node.js backend: ${proxyHttpUrl}`);
        const proxyResponse = await fetch(proxyHttpUrl, proxyFetchOptions);
